...
impl MetaVisitor {
    pub fn file_path(&self) -> Option<&str> {
        self.file_path.as_ref().map(|s| s.as_str())
    }
    pub fn source(&self) -> Option<&str> {
        self.source.as_ref().map(|s| s.as_str())
    }
    pub fn thread(&self) -> Option<&str> {
        self.thread.as_ref().map(|s| s.as_str())
    }
    ...
    N: for<'a> tracing_subscriber::fmt::FormatFields<'a> + 'static,
{
    fn format_event(
        ...
        write!(
            writer,
            "{}:{}",
            visitor.file_path().unwrap_or(meta.file().unwrap_or("unknown")).to_owned(),
            visitor.line_num.unwrap_or(meta.line().map(|l| l.to_string()).unwrap_or("-1".to_owned()))
        )?;
...
fn fill_string(left_aligned: bool, width: usize, mut s: String) -> String {
    ...
    if left_aligned {
        ...
        s.insert_str(0, &mut spaces.collect::<String>());
    }
    s
}
    ...
    N: for<'a> tracing_subscriber::fmt::FormatFields<'a> + 'static,
{
    fn format_event(
        ...
        let line = visitor.line_num.unwrap_or(meta.line().map(|l| l.to_string()).unwrap_or("-1".to_owned()));
