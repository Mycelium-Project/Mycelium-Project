...
export class NetworkTablePubbedTopic<T extends EnokiValue> {
  ...
  public setValue(value: T): void {
    ...
    invoke("plugin:nt|set_topic_value", {
  ...
  unpublish(): void {
    invoke("plugin:nt|unpublish_topic", {
      clientId: this.clientId,
      topic: this.topic,
    });
  }
...
export class NetworkTableSubscription {
  ...
  public async getSubbedData(): Promise<EnokiObject> {
    return invoke("plugin:nt|get_subbed_data", {
      clientId: this.clientId,
      topic: this.topic,
    });
  }
  ...
  public async getSubbedDataWithHistory(): Promise<EnokiObject> {
    let data = await invoke<EnokiObject>(
      "plugin:nt|get_subbed_data_with_history",
      {
        clientId: this.clientId,
        topic: this.topic,
        after: this.cached_history.timestamp,
      },
    );
...
export async function start_network_table_client(
  ip: number[],
  port: number,
  identity: string,
): Promise<NetworkTableClient> {
  return new NetworkTableClient(
    await invoke<NetworkTableClientId>("plugin:nt|start_network_table_client", {
      ip: ip,
      port: port,
      identity: identity,
    }),
...
export class NetworkTableClient {
  ...
  public createBoolTopic(topic: string): NetworkTablePubbedTopic<boolean> {
    ...
    let topic_obj = new NetworkTablePubbedTopic<boolean>(
      topic,
      "Bool",
      this.clientId,
    );
  ...
  public createDoubleTopic(topic: string): NetworkTablePubbedTopic<number> {
    ...
    let topic_obj = new NetworkTablePubbedTopic<number>(
      topic,
      "Double",
      this.clientId,
    );
  ...
  public createStringTopic(topic: string): NetworkTablePubbedTopic<string> {
    ...
    let topic_obj = new NetworkTablePubbedTopic<string>(
      topic,
      "String",
      this.clientId,
    );
  ...
  public createRawTopic(topic: string): NetworkTablePubbedTopic<Uint8Array> {
    ...
    let topic_obj = new NetworkTablePubbedTopic<Uint8Array>(
      topic,
      "Raw",
      this.clientId,
    );
  ...
  public creatBoolArrayTopic(
    topic: string,
  ): NetworkTablePubbedTopic<boolean[]> {
    if (this.topicMap.has(topic)) {
      throw new Error("Topic already exists");
    }
    let topic_obj = new NetworkTablePubbedTopic<boolean[]>(
  ...
  public createDoubleArrayTopic(
    topic: string,
  ): NetworkTablePubbedTopic<number[]> {
    if (this.topicMap.has(topic)) {
      throw new Error("Topic already exists");
    }
    let topic_obj = new NetworkTablePubbedTopic<number[]>(
  ...
  public createStringArrayTopic(
    topic: string,
  ): NetworkTablePubbedTopic<string[]> {
    if (this.topicMap.has(topic)) {
      throw new Error("Topic already exists");
    }
    let topic_obj = new NetworkTablePubbedTopic<string[]>(
  ...
  public isStopped(): Promise<boolean> {
    return invoke<boolean>("plugin:nt|is_network_table_client_stopped", {
      clientId: this.clientId,
    });
  }
}
